...
XIAOMI.namespace = function() {
    ...
    function() {
        var L = XIAOMI.lang,
            ...
            OB = {
                isArray: function(o) { return OP.toString.apply(o) === ARRAY_TOSTRING },
                isBoolean: function(o) { return "boolean" == typeof o },
                isFunction: function(o) { return "function" == typeof o || OP.toString.apply(o) === FUNCTION_TOSTRING },
                isNull: function(o) { return null === o },
                isNumber: function(o) { return "number" == typeof o && isFinite(o) },
                isObject: function(o) { return o && ("object" == typeof o || L.isFunction(o)) || !1 },
                isString: function(o) { return "string" == typeof o },
                isUndefined: function(o) { return "undefined" == typeof o },
                _IEEnumFix: -[1] ? function() {} : function(r, s) { var i, fname, f; for (i = 0; i < ADD.length; i += 1) fname = ADD[i], f = s[fname], L.isFunction(f) && f != OP[fname] && (r[fname] = f) },
                escapeHTML: function(html) { return html.replace(/[&<>"'\/`]/g, function(match) { return HTML_CHARS[match] }) },
                ...
                augmentProto: function(r, s) { if (!s || !r) throw new Error("Augment failed, verify dependencies."); var i, a = [r.prototype, s.prototype]; for (i = 2; i < arguments.length; i += 1) a.push(arguments[i]); return L.augmentObject.apply(this, a), r },
                dump: function(o, d) {
                    var i, len, s = [],
                        OBJ = "{...}",
                        FUN = "f(){...}",
                ...
                later: function(when, o, fn, data, periodic) {
                    ...
                    return L.isUndefined(data) || L.isArray(d) || (d = [data]), f = function() { m.apply(o, d || NOTHING) }, r = periodic ? setInterval(f, when) : setTimeout(f, when), { interval: periodic, cancel: function() { this.interval ? clearInterval(r) : clearTimeout(r) } }
                },
                isValue: function(o) { return L.isObject(o) || L.isString(o) || L.isNumber(o) || L.isBoolean(o) }
            };
        L.hasOwnProperty = OP.hasOwnProperty ? function(o, prop) { return o && o.hasOwnProperty && o.hasOwnProperty(prop) } : function(o, prop) { return !L.isUndefined(o[prop]) && o.constructor.prototype[prop] !== o[prop] }, OB.augmentObject(L, OB, !0), L.augment = L.augmentProto, XIAOMI.augment = L.augmentProto, XIAOMI.extend = L.extend
    }();;
(function() {
    function parse(s, buf, offset) {
        var i = buf && offset || 0,
            ii = 0;
        for (buf = buf || [], s.toLowerCase().replace(/[0-9a-f]{2}/g, function(oct) { 16 > ii && (buf[i + ii++] = _hexToByte[oct]) }); 16 > ii;) buf[i + ii++] = 0;
        return buf
    }
    ...
    if (!_rng && _global.crypto && crypto.getRandomValues) {
        ...
        _rng = function() { return crypto.getRandomValues(_rnds8), _rnds8 }
    }
    if (!_rng) {
        var _rnds = new Array(16);
        _rng = function() { for (var r, i = 0; 16 > i; i++) 0 === (3 & i) && (r = 4294967296 * Math.random()), _rnds[i] = r >>> ((3 & i) << 3) & 255; return _rnds }
    }
    ...
    var _seedBytes = _rng(),
        ...
        _lastNSecs = 0,
        uuid = v4;
    if (uuid.v1 = v1, uuid.v4 = v4, uuid.parse = parse, uuid.unparse = unparse, uuid.BufferClass = BufferClass, _global.define && define.amd) define(function() { return uuid });
    else if ("undefined" != typeof module && module.exports) module.exports = uuid;
    else {
        var _previousRoot = _global.uuid;
        uuid.noConflict = function() { return _global.uuid = _previousRoot, uuid }, _global.uuid = uuid
    }
}).call(this);;
XIAOMI.namespace("app.analytics"), XIAOMI.app.analytics = function(element, options) {
        this.options = { page: "首页", position: ["A1"], url: !0, JQelement: !1, isStat: !1, onlyStat: !1, pidPosition: ["头部区域"], statSign: "stat" };
        for (var p in options) this.options[p] = options[p];
        for (var _this = this, isURL = _this.options.url ? !0 : !1, trimAll = function(s) { try { return s.replace(/\s/g, "") } catch (e) { return s } }, bindFunction = function(links) {
                $.each(links, function(k, v) {
                    ...
                    aidString = trimAll(aidString), _this.options.onlyStat ? $(v).attr({ "data-stat-pid": pidParmas, "data-stat-aid": aidString }) : _this.options.isStat ? $(v).attr({ "data-stat-pid": pidParmas, "data-stat-aid": aidString, onclick: "_hmt.push(['_trackEvent'," + parmas + "]);" }) : $(v).attr("onclick", "_hmt.push(['_trackEvent'," + parmas + "]);")
                })
            }, i = 0; i < element.length; i++) {
            var links = _this.options.JQelement ? element[i] : $(element[i]).find("a");
            bindFunction(links)
        }
    }, XIAOMI.namespace("app.updateMiniCart, app.addShopCart, app.addShopCartEvent"), XIAOMI.app.updateMiniCart = function() { var num = XIAOMI.app.cookie("xm_user_www_num"); return num && parseFloat(num) > 0 ? ($(".J_cartNum").html("(" + num + ")").show(), num) : ($(".J_cartNum").html("").hide(), 0) }, XIAOMI.app.addShopCart = function(gid, callback, obj) {
        if (gid && "function" == typeof callback) {
            var ajaxUrl = XIAOMI.GLOBAL_CONFIG.cartSite + "/cart/add/" + gid;
            $.ajax({ url: ajaxUrl, dataType: "jsonp", jsonp: "jsonpcallback", success: function(data) { callback(data, obj), XIAOMI.app.updateMiniCart() } })
        }
        ...
        $.extend(op, options || {}), $(op.obj).on("click", function() {
            ...
            if ("false" === isDisabled) {
                ...
                if (dmSkuArr !== !1 && "object" == typeof dmSkuArr && (isToDm = $.inArray(gid, dmSkuArr) >= 0 ? !0 : !1), isToDm === !0) {
                    ...
                    XIAOMI.app.cookie("userId") && XIAOMI.app.cookie("xm_order_btauth") ? XIAOMI.app.dmFun.init({ sku: gid, callback: op.callback, obj: $(this), overCallback: op.overCallback }) : XIAOMI.app.cookie("userId") ? (dmLogin._proxyiframe(), XIAOMI.app.dmFun.init({ sku: gid, callback: op.callback, obj: $(this), overCallback: op.overCallback })) : window.location.href = XIAOMI.GLOBAL_CONFIG.orderSite + "/site/login?redirectUrl=" + location.href
    ...
    function() { document.write = document.writeln = function() {}; for (var method, noop = function() {}, methods = ["assert", "clear", "count", "debug", "dir", "dirxml", "error", "exception", "group", "groupCollapsed", "groupEnd", "info", "log", "markTimeline", "profile", "profileEnd", "table", "time", "timeEnd", "timeStamp", "trace", "warn"], length = methods.length, console = window.console = window.console || {}; length--;) method = methods[length], console[method] || (console[method] = noop) }(), XIAOMI.namespace("app.cookie"), XIAOMI.app.cookie = function(key, value, options) {
        ...
        init: function(options) {
            var _this = this;
            _this.config = { sku: null, callback: null, obj: null, overCallback: null, sourceVal: "bigtap" }, _this.inTheQueue = !1, $.extend(_this.config, options), _this.startQueue(), _this.getDmSys(), $("#xmDmReload").off().on("click", function() { _this.config.obj.trigger("click") })
        },
        getDmSys: function() {
            var _this = this,
            ...
            $.ajax({
                ...
                timeout: 3e4,
                error: function() { return _this.inTheQueue ? (_this.stopQueue(), void $("#xmDmError").modal({ show: !0, backdrop: "static" })) : !1 },
        ...
        getShopCart: function(token) {
            ...
            $.ajax({ type: "GET", url: addCart, dataType: "jsonp", jsonp: "jsonpcallback", success: function(data) { _this.stopQueue(), "function" == typeof _this.config.callback && _this.config.callback(data, _this.config.obj), XIAOMI.app.updateMiniCart() } })
        },
        startQueue: function() {
            var _this = this;
            $("#xmDmError").modal("hide"), $("#xmDmQueue").modal({ show: !0, backdrop: "static" }), _this.queueAnimate(), _this.inTheQueue = !0, $("#xmDmQueue").on("hidden.bs.modal", function() { _this.stopQueue(), _this.config.obj.attr("data-disabled", "false") })
        },
        stopQueue: function() { $("#xmDmQueue").modal("hide"), this.inTheQueue = !1, this.getDmTimer && clearTimeout(this.getDmTimer), window.queueAnimateTimer && clearInterval(queueAnimateTimer) },
        queueAnimate: function() {
            var flag = 0,
                speed = 200,
                loop = function() { flag -= 90, -900 >= flag && (flag = 0), $("#mituWalking").css({ "background-position": flag + "px 0" }) };
            window.queueAnimateTimer = setInterval(loop, speed)
        }
    }, XIAOMI.namespace("app.footer"), XIAOMI.app.footer = {
        init: function() {
            var _this = this;
            _this.retinizer(), _this.modalCommonTrack(), _this.siteList(), XIAOMI.app.analytics([".site-topbar", ".open-buy-info", ".site-logo", ".cart-section", ".footer-links", ".footer-info", ".footer-service", ".hot-words", ".J_menuNavMain"], { position: ["A", "顶部链接", "logo", "A", "about", "footer", "service", "hotwords", "A"], url: !1, isStat: !0, pidPosition: ["头部顶部站点导航", "头部开放购买", "头部LOGO", "头部购物车", "尾部帮助中心", "尾部版权", "尾部服务", "头部搜索热词", "头部导航菜单"] }), $(".J_item-detail-more").parent().addClass("children-list-more")
        },
        retinizer: function() {
            var img = new Image;
        ...
        siteList: function() { $(".J_globalList").on({ mouseenter: function() { $(this).find(".global-site-list").show() }, mouseleave: function() { $(this).find(".global-site-list").hide() } }) },
        modalRender: function(obj) { obj.css({ width: obj.attr("data-width"), height: obj.attr("data-height") }).modal({ backdrop: !0, keyboard: !0, show: !0 }) },
        modalCommonTrack: function() {
            var _this = this,
                $modalWeixin = $("#J_modalWeixin"),
                $modalGlobalSites = $("#J_modal-globalSites");
            $(".J_modalWeixin").on("click", function() { _this.modalRender($modalWeixin) }), $(".J-modal-globalSites").on("click", function() { _this.modalRender($modalGlobalSites) })
        },
        ...
        getData: function(parent, target, sId) {
            var _this = this;
            $.ajax({ type: "GET", url: _this.getUrl, data: "parent=" + parent, dataType: "json", success: function(data) { data && _this.formatData(data, target, sId) } })
        },
        ...
        init: function() {
            var _this = this,
                timeOut = null;
            return "undefined" != typeof miniCartDisable && miniCartDisable ? !1 : "undefined" != typeof XIAOMI.GLOBAL_CONFIG.closeMiniCart && "1" === XIAOMI.GLOBAL_CONFIG.closeMiniCart ? !1 : (_this.elmCartBtn.on({ mouseenter: function() { clearTimeout(timeOut), $(this).hasClass("mini-cart-on") || ($(this).addClass("mini-cart-on"), _this.show(), _this.getData()) }, mouseleave: function() { timeOut = setTimeout(function() { _this.close() }, _this.speed) } }), _this.elmCartList.on({ mouseenter: function() { clearTimeout(timeOut) }, mouseleave: function() { timeOut = setTimeout(function() { _this.close() }, _this.speed) } }), void _this.elmCartList.on("click", ".delItem", function(e) {
        ...
        delGoods: function(gId) {
            var _this = this;
            $.ajax({ type: "POST", url: XIAOMI.GLOBAL_CONFIG.cartSite + "/cart/delete/" + gId, dataType: "jsonp", jsonp: "jsonpcallback", cache: "false", success: function(data) { 1 === data.code ? (_this.getData(), XIAOMI.app.updateMiniCart()) : alert(data.msg) } })
        },
        getData: function() {
            var _this = this;
            $.ajax({ type: "POST", url: XIAOMI.GLOBAL_CONFIG.cartSite + "/cart/miniNew", dataType: "jsonp", jsonp: "jsonpcallback", cache: !1, success: function(data) { _this.formatData(data), XIAOMI.app.updateMiniCart() } })
        },
    ...
    }, XIAOMI.namespace("app.miniLogin"), XIAOMI.app.miniLogin = function() { this.orderSite = XIAOMI.GLOBAL_CONFIG.orderSite || "http://order.mi.com", this.quickLoginUrl = XIAOMI.GLOBAL_CONFIG.quickLoginUrl }, XIAOMI.app.miniLogin.prototype = {
        ...
        _proxyiframe: function() {
            ...
            $(document.body).append(iframebody), $("iframe[name='proxy']").load(function() { $("iframe[name='proxy']").remove() })
        },
        _toLogin: function(rel) {
            function appendTobody(url) {
                ...
                $("#loginBox-con").css({ height: boxHeight }).html(content), $("#loginBox").css({ width: boxWidth, "margin-left": -boxWidth / 2 }).modal({ backdrop: !0, keyboard: !0, show: !0 })
            }

            function goLocation() { clearInterval(timer), rel ? window.location.href = rel : window.location.reload(!0) }
            ...
            $.ajax({ url: _this.orderSite + "/site/loginurl?followup=" + shref, dataType: "jsonp", jsonp: "jsonpcallback", success: function(data) { appendTobody(data.url) }, Complete: function() {}, error: function() { appendTobody() } });
            var timer = window.setInterval(checkHash, 1e3)
        },
        init: function() { var _this = this; - 1 !== window.location.pathname.indexOf("cart") && XIAOMI.app.cookie("userId") && !XIAOMI.app.cookie("xm_order_btauth") && _this._proxyiframe(), $("[data-needLogin='true']").on("click", function() { var redirectUrl = encodeURIComponent(window.location.href); return window.location.href = _this.orderSite + "/site/login?ac=1&redirectUrl=" + redirectUrl, !1 }) },
        _isLogined: function(rel, e) { XIAOMI.app.cookie("xm_order_btauth") || (XIAOMI.app.cookie("userId") ? this._proxyiframe() : (e && XIAOMI.lang.preventDefault(e), this._toLogin(rel))) },
        autoExec: function(rel) { this._isLogined(rel) }
        ...
        function init(elm, opt) {
            function bindToggleEvent(curIndex, show) { options.delay > 0 && "click" !== options.triggerEvent ? show ? (timeoutToggle && window.clearTimeout(timeoutToggle), timeoutToggle = window.setTimeout(function() { toggleMenu($elm, curIndex, !0, options) }, options.delay)) : (window.clearTimeout(timeoutToggle), timeoutToggle = window.setTimeout(function() { toggleMenu($elm, curIndex, !1, options) }, options.delay)) : show ? toggleMenu($elm, curIndex, !0, options) : toggleMenu($elm, curIndex, !1, options), "function" == typeof options.callback && options.callback($elm, curIndex, options) }
            var options, $menu, timeoutToggle, $elm = $(elm);
            return 0 === $elm.length ? $elm : $elm.length > 1 ? ($elm.each(function() { init($(this), opt) }), this) : (options = $.extend({}, defaults, opt), $menu = $elm.children(options.menuSelector), "hover" === options.triggerEvent ? $menu.on({ mouseenter: function() { bindToggleEvent($menu.index($(this)), !0) }, mouseleave: function() { bindToggleEvent($menu.index($(this)), !1) } }) : "click" === options.triggerEvent && $menu.on("click", function(e) { e.preventDefault(), $(this).hasClass("toggled") ? ($(this).removeClass("toggled"), bindToggleEvent($menu.index($(this)), !1)) : ($menu.removeClass("toggled"), $(this).addClass("toggled"), bindToggleEvent($menu.index($(this)), !0)) }), void(window.ActiveXObject && !window.XMLHttpRequest && $menu.find(options.submenuSelector).find("a").on("click", function() { window.location.href = $(this).attr("href") })))
        }
        var defaults;
        return defaults = { menuSelector: ".menu", submenuSelector: ".children", triggerEvent: "hover", effect: "toggle", delay: 200, speed: 200, callback: function() {} }, { toggleMenu: toggleMenu, init: init }
    }(), XIAOMI.app.navCategory = function() {
        function lazyLoadImg(elm) {
            var $elm = $(elm);
            $elm.each(function() { $(this).hasClass("img-loaded") || "string" != typeof $(this).attr("data-src") || $(this).addClass("img-loaded").attr({ src: $(this).attr("data-src"), srcset: $(this).attr("data-srcset") }) })
        }
        ...
        function bindChildrenList() {
            ...
            $categoryList.find(".nav-category-list").children("li").on({
                ...
                mouseleave: function() {
                    var $this = $(this);
                    window.clearTimeout(timeoutChildrenList), timeoutChildrenList = window.setTimeout(function() { $this.removeClass("current") }, 100)
                }
            })
        }
        ...
        function init() {
            var isCategoryStatic = "undefined" != typeof isCategoryToggled ? isCategoryToggled : !1,
                MI_SITES = { orderSite: "http://order.mi.com", wwwSite: "http://www.mi.com", cartSite: "http://cart.mi.com", itemSite: "http://item.mi.com", listSite: "http://list.mi.com", searchSite: "http://search.mi.com", mySite: "http://my.mi.com" },
            ...
            $.ajax({
                ...
                success: function() {
                    ...
                    if ($("#J_categoryContainer").find(".nav-category-list").html(templateCategory(categoryTree)), XIAOMI.app.analytics([$(".nav-category-content > a"), $(".nav-category-content .links").find("a"), $(".nav-category-content .children-list").find("a"), $(".nav-category-content .children-special-section").find("a")], { page: "分类导航", position: ["A", "B", "C", "D"], JQelement: !0, url: !1, isStat: !0, pidPosition: ["头部分类导航第一级", "头部分类导航第二级", "头部分类导航第三级", "头部分类导航热门商品"] }), isCategoryStatic) toggleCategoryList("static");
                    else {
                        $categoryTriggerBtn.append('<i class="iconfont">&#xe608;</i>');
                        var timeoutCategory;
                        $categoryContainer.on({ mouseenter: function() { window.clearTimeout(timeoutCategory), $(this).hasClass("nav-category-toggled") || (timeoutCategory = window.setTimeout(function() { toggleCategoryList("show") }, 200)) }, mouseleave: function() { window.clearTimeout(timeoutCategory), $(this).hasClass("nav-category-toggled") && (timeoutCategory = window.setTimeout(function() { toggleCategoryList("hide") }, 200)) } })
                    }
                    bindChildrenList()
                }
            })
        ...
        getResult: function(k) {
            var _this = this;
            k && $.ajax({ type: "GET", url: "http://search.mi.com/search/tips?keyword=" + encodeURIComponent(k), dataType: "jsonp", jsonp: "jsonpcallback", jsonpCallback: "xmsearch", cache: !0, success: function(data) { data.length > 0 ? _this.inputObj.val() && _this.formatData(data) : _this.removeList() } })
        },
        formatData: function(data) {
        ...
        removeList: function() { $("#J_keywordList").addClass("hide").children(".result-list").html("") },
        submit: function() {
            var _this = this;
            $("#J_searchForm").on("submit", function() { var val = $.trim(_this.inputObj.val()); return "" !== val && (val = val.replace(/[\-_'=+|\\\/]/g, " "), window.location.href = "http://search.mi.com/search_" + encodeURIComponent(val)), !1 })
        }
        ...
        init: function(elm, opt) {
            function reorderList(elm) { elm.insertBefore(elm.closest("ul").children("li")[0]), elmText.text(elm.text()) }
            ...
            }), void elmWrap.on({ mouseover: function() { elmWrap.addClass("toggled"), elmList.show() }, mouseout: function() { elmWrap.removeClass("toggled"), elmList.hide() } }))
        }
...
XIAOMI.namespace("setRecommend"), XIAOMI.app.setRecommend = function(options) {
    var option = { obj: $("#item-recommend-box"), tmpl: $("#item-recommend-template"), url: XIAOMI.GLOBAL_CONFIG.sideBarApiUrl + "cart/get/id/1134900361,1134900465,1135000458,1140400001/callback/itemRecommend/cid/2", max: 5, min: 0, callback: null };
...
! function($) {
    ...
    Modal.prototype = {
        ...
        hideModal: function() {
            var that = this;
            this.$element.hide(), this.backdrop(function() { that.removeBackdrop(), that.$element.trigger("hidden") })
        },
        ...
        backdrop: function(callback) {
            ...
            if (this.isShown && this.options.backdrop) {
                var doAnimate = $.support.transition && animate,
                    ...
                    pageWidth = "100%";
                if (this.$backdrop = $('<div class="modal-backdrop ' + animate + '" />').appendTo(document.body), this.$backdrop.css({ width: pageWidth, height: pageHeight }).click("static" == this.options.backdrop ? $.proxy(this.$element[0].focus, this.$element[0]) : $.proxy(this.hide, this)), doAnimate && this.$backdrop[0].offsetWidth, this.$backdrop.addClass("in"), !callback) return;
...
! function() {
    ...
    function encodeHTMLSource() {
        var encodeHTMLRules = { "&": "&#38;", "<": "&#60;", ">": "&#62;", '"': "&#34;", "'": "&#39;", "/": "&#47;" },
            matchHTML = /&(?!#?\w+;)|<|>|"|'|\//g;
        return function() { return this ? this.replace(matchHTML, function(m) { return encodeHTMLRules[m] || m }) : this }
    }

    function resolveDefs(c, block, def) { return ("string" == typeof block ? block : block.toString()).replace(c.define || skip, function(m, code, assign, value) { return 0 === code.indexOf("def.") && (code = code.substring(4)), code in def || (":" === assign ? (c.defineParams && value.replace(c.defineParams, function(m, param, v) { def[code] = { arg: param, text: v } }), code in def || (def[code] = value)) : new Function("def", "def['" + code + "']=" + value)(def)), "" }).replace(c.use || skip, function(m, code) { c.useParams && (code = code.replace(c.useParams, function(m, s, d, param) { if (def[d] && def[d].arg && param) { var rw = (d + ":" + param).replace(/'|\\/g, "_"); return def.__exp = def.__exp || {}, def.__exp[rw] = def[d].text.replace(new RegExp("(^|[^\\w$])" + def[d].arg + "([^\\w$])", "g"), "$1" + param + "$2"), s + "def.__exp['" + rw + "']" } })); var v = new Function("def", "return " + code)(def); return v ? resolveDefs(c, v, def) : v }) }

    function unescape(code) { return code.replace(/\\('|\\)/g, "$1").replace(/[\r\t\n]/g, " ") }
    var global, doT = { version: "1.0.0", templateSettings: { evaluate: /\{\{([\s\S]+?\}?)\}\}/g, interpolate: /\{\{=([\s\S]+?)\}\}/g, encode: /\{\{!([\s\S]+?)\}\}/g, use: /\{\{#([\s\S]+?)\}\}/g, useParams: /(^|[^\w$])def(?:\.|\[[\'\"])([\w$\.]+)(?:[\'\"]\])?\s*\:\s*([\w$\.]+|\"[^\"]+\"|\'[^\']+\'|\{[^\}]+\})/g, define: /\{\{##\s*([\w\.$]+)\s*(\:|=)([\s\S]+?)#\}\}/g, defineParams: /^\s*([\w$]+):([\s\S]+)/, conditional: /\{\{\?(\?)?\s*([\s\S]*?)\s*\}\}/g, iterate: /\{\{~\s*(?:\}\}|([\s\S]+?)\s*\:\s*([\w$]+)\s*(?:\:\s*([\w$]+))?\s*\}\})/g, varname: "it", strip: !0, append: !0, selfcontained: !1 }, template: void 0, compile: void 0 };
    "undefined" != typeof module && module.exports ? module.exports = doT : "function" == typeof define && define.amd ? define(function() { return doT }) : (global = function() { return this || (0, eval)("this") }(), global.doT = doT), String.prototype.encodeHTML = encodeHTMLSource();
    ...
    doT.template = function(tmpl, c, def) {
        ...
        try { return new Function(c.varname, str) } catch (e) { throw "undefined" != typeof console && console.log("Could not create a template function: " + str), e }
